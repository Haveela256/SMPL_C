...
public class CA_SignIn_page {
    ...
    public void emailField(String EmailAddress, String ErrorMessage) {
        ...
        } else if (driver.findElement(passwordFieldPath).isDisplayed()) {
            driver.findElement(passwordFieldPath).click();
            driver.findElement(emailRequiredPath).isDisplayed();
            Assert.assertEquals(ErrorMessage, emailRequiredPath);
            System.out.println(emailRequiredPath);

        }
    }


    public void passwordField(String Password, String ErrorMessage) {
        if (driver.findElement(passwordFieldPath).isDisplayed()) {
            driver.findElement(passwordFieldPath).sendKeys(Password);
        } else if (driver.findElement(passwordFieldPath).isDisplayed()) {
            driver.findElement(loginbuttonPath).click();
            driver.findElement(passwordRequiredPath).isDisplayed();
            Assert.assertEquals(ErrorMessage, passwordFieldPath);
            System.out.println(passwordFieldPath);

        }
    }

    public void eyeIcon() {
        driver.findElement(passwordFieldPath).sendKeys("Vassar@123");
    ...
    public void forgetPassword() {
        driver.findElement(forgetPasswordPath).click();
